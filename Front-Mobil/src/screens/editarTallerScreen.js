...
const EditarTallerScreen = ({ navigation, route }) => {
    const { taller, usuario, idUsuario } = route.params || {};
    
    // ========== ESTADOS ==========
    const [titulo, setTitulo] = useState(taller?.titulo || '');
    const [descripcion, setDescripcion] = useState('');
    const [idEspacio, setIdEspacio] = useState(taller?.id_espacio || null);
    const [espacioSeleccionado, setEspacioSeleccionado] = useState(null);
    const [fecha, setFecha] = useState(taller?.fecha ? new Date(taller.fecha) : new Date());
    const [horaInicio, setHoraInicio] = useState(() => {
        if (taller?.horaInicio) {
            const [horas, minutos] = taller.horaInicio.split(':');
    ...
    const cargarEspacios = async () => {
        try {
            ...
            if (taller?.lugar) {
                const espacioEncontrado = data.find(e => e.nombre === taller.lugar);
                if (espacioEncontrado) {
                    setIdEspacio(espacioEncontrado.id);
                    setEspacioSeleccionado(espacioEncontrado);
                }
            }
        } catch (error) {
    ...
    const handleGuardarCambios = async () => {
        ...
        try {
            ...
            const response = await fetch(`${API_BASE_URL}/docente/taller/actualizar?${params.toString()}`, {
                method: 'PUT',
            ...
            if (response.ok) {
                ...
                Alert.alert('Error', data.detail || 'No se pudo actualizar el taller');
            }
        } catch (error) {
        ...
        <SafeAreaView style={styles.container}>
            ...
            <ScrollView contentContainerStyle={styles.scrollContent}>
                ...
                <View style={styles.formContainer}>
                    <Text style={styles.title}>Editar Taller</Text>
                    ...
                    <TouchableOpacity 
                        style={[styles.saveButton, cargando && styles.buttonDisabled]}
...
const styles = StyleSheet.create({
    ...
    // Modal de selección de espacios
    modalOverlay: {
        flex: 1,
...
export default EditarTallerScreen;