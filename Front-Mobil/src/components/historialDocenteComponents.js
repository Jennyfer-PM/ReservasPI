...
const HistorialDocenteComponents = ({ visible, onClose, idUsuario }) => {
    ...
    const fetchHistorial = async () => {
        ...
        try {
            let docId = docenteId;
            if (!docId) {
                docId = await obtenerIdDocente();
            }
            
            if (docId) {
                const response = await fetch(
                    `${API_BASE_URL}/docente/talleres/historial/${docId}?mes=${mes}&anio=${anio}`
                );
                const data = await response.json();
                setTalleres(data);
            }
        } catch (error) {
            console.error("Error historial docente:", error);
        } finally {
            setCargando(false);
        }
    };
            ...
            <View style={styles.overlay}>
                <View style={styles.container}>
                    <View style={styles.header}>
                        <Text style={styles.title}>Historial de talleres</Text>
                            ...
                            ListEmptyComponent={
                                <View style={styles.emptyContainer}>
                                    ...
                                    <Text style={styles.emptyText}>
                                        No hay talleres para este periodo
                                    </Text>
...
export default HistorialDocenteComponents;